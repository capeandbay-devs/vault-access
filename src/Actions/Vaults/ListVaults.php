...
class ListVaults
{
    ...
    public function asController(Request $request)
    {
        ...
        if(config('vault-access.enable_caching'))
        {
            ...
            if(!is_null($user))
            {
                ...
                $results = Cache::remember($user->id.'-list-o-vaults', (60 * 5), function() use ($_this, $data){
                    return $_this->handle($data['token']);
                });
            }

            if(!$results)
            {
                Cache::forget($user->id.'-list-o-vaults');
            }
        }
        else
        {
            $results = $this->handle($data['token']);
        }
    ...
    {
        $results = false;
        $url = config('vault-access.1password_connect_url').'/v1/vaults';
    ...
    public function jsonResponse($result)
    {
        ...
        if($result)
        {
            $results = ['success' => true, 'vaults' => $result];
            $code = 200;
        }
